...
function collectStats(){
    ...
    async.parallel({
        ...
        let m = Object.keys(liveConnections).length, n = Object.keys(addressConnections).length;

        if ( error || ( m + n > 0 ) ) log('info', logSystem, 'Stat collection finished: %d ms redis, %d ms daemon', [redisFinished - startTime, daemonFinished - startTime]);
...
function broadcastLiveStats( m, n ){
    log('info', logSystem, 'Broadcasting to %d visitors and %d address lookups', [ m, n ]);
