...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        if (!currentBlockTemplate || result.height > currentBlockTemplate.height){
            log('info', logSystem, 'New block to mine at height %d; difficulty == %d.', [result.height, result.difficulty]);
...
Miner.prototype = {
    ...
    getTargetHex: function(){
        ...
        var buffReversed = padded.slice(0, 4).reverse();
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate){
    ...
    log('info', logSystem, 'Accepted %s share with difficulty %d/%d from %s@%s.', [shareType, job.difficulty, shareDiff, miner.login, miner.ip]);
...
function handleMinerMethod(method, params, ip, portData, sendReply, pushMessage){
    ...
    switch(method){
        case 'login':
            ...
            // dmjp: WTF?  why 2 args? what is the supposed to do?  if (!utils.isValidAddress(params.login, config.poolServer.poolAddress[0])){
            ...
            log('info', logSystem, 'Miner %s@%s connected.',  [params.login, miner.ip]);
            break;
        case 'getjob':
            if (!miner){
